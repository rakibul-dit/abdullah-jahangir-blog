...
export const getAllQuestions = async (obj) => {
	...
	if (categoryId == "all") {
		...
		dataSet = qna.filter((item) => item.category == categoryId);
	}
...
export const getBooksByCategory = async (obj) => {
	...
	if (categoryId == "all") {
		...
		dataSet = books.filter((item) => item.category == categoryId);
	}

	const totalItems = dataSet.length;
	let maxResult = constants.BOOKS_PER_REQ ? constants.BOOKS_PER_REQ : 1;
	// let maxResult = 1000;
	const numberOfPages = Math.ceil(totalItems / maxResult);
	// console.log(numberOfPages);
...
export const getSomeImpLects = async () => {
	const url = `${youtube.url}/playlistItems?key=${youtube.key}&part=snippet&playlistId=${youtube.uploadPlaylistID}&maxResults=${constants.SOME_IMPORTANT_LECTURES}`;
	const lectures = await getYoutubeVideoListByUrl(url);
...
const quotes = [
	{
		...
		text: "যখন উপাসনা শেষ হবে, তোমরা জমিনে ছড়িয়ে পড়বে এবং আল্লাহর দান অনুসন্ধানে ব্যাপৃত থাকবে",
		author: "আল কুরআন",
	},
	{
		id: 2,
		image: "/img/slider/02.jpg",
		text: "এমনিভাবে আমি তোমাদেরকে মধ্যপন্থী সম্প্রদায় করেছি যাতে করে তোমরা সাক্ষ্যদাতা হও মানবমন্ডলীর জন্যে এবং যাতে রসূল সাক্ষ্যদাতা হন তোমাদের জন্য",
		author: "আল কুরআন",
	},
	{
		id: 3,
		image: "/img/slider/03.jpg",
		text: "তিনিই প্রথম, তিনিই সর্বশেষ, তিনিই প্রকাশমান ও অপ্রকাশমান এবং তিনি সব বিষয়ে সম্যক পরিজ্ঞাত",
		author: "আল কুরআন",
	},
];
