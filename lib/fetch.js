...
export const getHomeLectures = async () => {
  ...
  return {
	videoLists: lectures.videoLists.videos.slice(7, 11),
...
export const getYoutubeVideoDetailsByUrl = async (url) => {
  ...
  if (typeof data.items != 'undefined') {
	...
	image = (typeof data.items[0].snippet.thumbnails.high !== 'undefined') ? data.items[0].snippet.thumbnails.high.url : ''
	viewCount = (typeof data.items[0].statistics !== 'undefined') ? data.items[0].statistics.viewCount : ''
  }
...
const replaceYoutubeKeyFromUrl = (url, key) => {
  const newUrl = url.replace(new RegExp("key([^&]*)&", "gm"), "key="+ key +"&")
  return newUrl
}
