...
export default function LectureList({ initialVideos, initPlaylistId, playlists }) {
  const ref = useRef()
  const catRef = useRef()
  ...
  const { data, error, mutate, size, setSize, isValidating } = useSWRInfinite(
    ...
    fetcher, { initialData: initialVideos, revalidateOnMount: true }
  )
  ...
  const numberOfPages = data?.[0]?.length !== 0 ? data[0].videoLists.numberOfPages : 0
